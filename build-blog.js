#!/usr/bin/env node
/**
 * Blog Build Script — Palm Beach Warehouses
 *
...
const SITE_URL = 'https://palmbeachwarehouses.com';
...
function generateIndex(articles) {
  if (!fs.existsSync(INDEX_TEMPLATE_PATH)) {
    // Generate a simple index if no template exists
    console.log('  No _index-template.html found, generating default index.');
  }
  ...
  if (fs.existsSync(INDEX_TEMPLATE_PATH)) {
    ...
    // Fallback — should not be needed once template exists
    indexHtml = `<!DOCTYPE html><html><head><title>Blog | Palm Beach Warehouses</title></head><body>${articleCards}</body></html>`;
  }
...
function updateSitemap(articles) {
  ...
  blogEntries += `
    <url>
        ...
        <lastmod>${articles[0]?.date || '2026-03-08'}</lastmod>
